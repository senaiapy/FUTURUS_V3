...
export default function Referral() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setReferral(`${window.location.origin}/referral?ref=${res.data.code}`);
  ...
  const activeReferrals = referral?.filter(
    ...
    <div className="w-full max-w-[1200px] mx-auto px-6 py-12 flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        >
          ...
          <p className="text-[#00b4d8] text-2xl font-bold">
            {referral?.length}
          </p>
        </motion.div>
        <motion.div
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#00b4d8] text-2xl font-bold">
              {referral?.reduce((sum, i) => sum + i.fee, 0)} FUT
            </span>
          </div>
        </motion.div>
      </div>
