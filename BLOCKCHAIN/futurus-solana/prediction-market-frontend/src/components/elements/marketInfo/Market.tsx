    ...
    (async () => {
      ...
      if (pathname === "/fund") {
        marketData = await axios.get(
          `${url}api/market/get?page=${currentPage}&limit=10&marketStatus=PENDING&marketField=${selectedCategory === "Sports" ? 1 : 0}`,
        );
      } else if (pathname === "/") {
        marketData = await axios.get(
          `${url}api/market/get?page=${currentPage}&limit=10&marketStatus=ACTIVE&marketField=${selectedCategory === "Sports" ? 1 : 0}`,
        );
      }
