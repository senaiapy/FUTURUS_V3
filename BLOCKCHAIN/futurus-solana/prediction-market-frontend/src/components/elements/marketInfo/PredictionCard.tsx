    ...
    try {
      ...
      if (res.status === 200) {
        ...
        const marketData = await axios.get(
          `${url}api/market/get?page=${currentPage}&limit=10&marketStatus=ACTIVE&marketField=0`,
        );
    ...
    >
      <div className="self-stretch flex flex-col justify-start items-start gap-2">
        ...
        <div className="self-stretch inline-flex justify-start items-start gap-4">
          ...
          <img
            className="lg:w-14 lg:h-14 w-12 h-12 rounded-lg"
        ...
        <div className="self-stretch grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-1">
            ...
            <div className="text-white text-sm font-semibold">
              {markets[index].totalInvestment} FUT
            </div>
          </div>
          <div className="flex flex-col gap-1">
            <div className="text-[#838587] text-sm font-semibold">
              Time Remaining
            </div>
        ...
        <div className="self-stretch flex flex-col gap-2">
          <div className="flex justify-between items-center">
            ...
            <div className="text-white text-sm font-semibold">
              {Math.floor(
                (markets[index].playerACount /
                  (markets[index].playerACount + markets[index].playerBCount)) *
                  100,
              )}
              %
            </div>
          ...
          <ProgressBar
            yesPercentage={
              markets[index].playerACount + markets[index].playerBCount === 0
                ? 50
                : Math.floor(
                    (markets[index].playerACount /
                      (markets[index].playerACount +
                        markets[index].playerBCount)) *
                      100,
                  )
            }
          />
      ...
      <div className="self-stretch inline-flex justify-start items-center gap-3">
        ...
        >
          ...
          <span className="text-[#3fd145] text-lg font-bold font-satoshi leading-7">
            Yes
          </span>
        </motion.button>
        <motion.button
