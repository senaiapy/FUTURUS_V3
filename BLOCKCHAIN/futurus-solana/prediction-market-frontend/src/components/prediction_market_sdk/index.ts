...
const LAMPORTS_PER_FUT = LAMPORTS_PER_SOL;
...
let futConnection = new Connection(
  process.env.NEXT_PUBLIC_FUTANA_RPC_URL || "https://api.devnet.solana.com",
  { commitment: "confirmed" }
);
// let provider: anchor.Provider = new anchor.AnchorProvider(futConnection, wallet!, {});
...
let program: anchor.Program = new anchor.Program(idl as anchor.Idl, { connection: futConnection });
...
export const createMarket = async (param: CreateMarketType) => {
  ...
  let provider: anchor.Provider = new anchor.AnchorProvider(futConnection, param.anchorWallet, {
    skipPreflight: true,
    commitment: "confirmed",
  });
  ...
  let latestBlockHash = await futConnection.getLatestBlockhash(
    futConnection.commitment
  );
  ...
  const sim = await futConnection.simulateTransaction(vtx);
  ...
  const createV0Tx = await futConnection.sendTransaction(signedTx);
  ...
  // const sig = await futConnection.sendTransaction(creatTx, [param.creator], {
  //   skipPreflight: true,
  // });
...
export const depositLiquidity = async (param: DepositeLiquidityType) => {
  ...
  let tx = await program.methods.addLiquidity(new BN(param.amount * LAMPORTS_PER_FUT))
    .accounts({
      user: investerPubkey,
...
export const marketBetting = async (param: BetType) => {
  ...
  let [playerTokenAccount, player_ata_instruction] = await getOrCreateATAInstruction(tokenPubkey, playerPubkey, futConnection );

  // FUT tokens accounts
  let [playerFutAccount, player_fut_ata_instruction] = await getOrCreateATAInstruction(FUT_MINT, playerPubkey, futConnection );
  let marketFutAccount = await getAssociatedTokenAccount(market, FUT_MINT);
  let feeAuthorityFutAccount = await getAssociatedTokenAccount(new PublicKey(feeAuthority), FUT_MINT);
  ...
  const tx = await program.methods.createBet({
    ...
    user: playerPubkey,
    futMint: FUT_MINT,
    userFutAccount: playerFutAccount,
    marketFutAccount: marketFutAccount,
    feeAuthorityFutAccount: feeAuthorityFutAccount,
  ...
  if (player_fut_ata_instruction) {
    betTx.add(player_fut_ata_instruction);
  }
  ...
  const info = await (program.account as any).market.fetch(market, "confirmed");
  //@ts-ignore
...
//     user: auth.publicKey,
//     market,
//     feedAggregator: new PublicKey(FUT_USDC_FEED),
...
export const fetchMarketInfo = async (pda: PublicKey) => {
  try {
    const info = await (program.account as any).market.fetch(pda, "confirmed");
