...
✅ **Deposite liquidity** – Users can deposite fut before start betting and once the liqudity amount reaches special amount they can start betting "Yes" and "No".
...
- Users can deposite fut to market which is already created
```typescript
...
...
```
- Once fut amount reaches specific value it changes market status. So users can start betting.
```rust
...
msg!("🎫Current FUT/USD price 🎫{}", current_sol_price);
