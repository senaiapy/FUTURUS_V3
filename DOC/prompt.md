...
Create a component example page that will list all of our UI components with all their variants so we can visually see how they turned out. and place this page in @app.
  ...
  install remote server in ip 51.83.103.50 over user gamba and passord @, in path /www/wwwroot/prevejo.com.br, with aapanel with reverse proxy and docker compose containers, but markests not seeded or not permission over backend over cors, analyze and fix this, and in necessary fix local .env or code or script server-uploaded