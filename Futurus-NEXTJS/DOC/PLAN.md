...
CREATE OR REPLACE FUNCTION resolve_group_payouts(
    ...
    market_outcome VARCHAR(10), -- 'YES' or 'NO'
    payout_per_share DECIMAL(18, 8) -- Value paid for each winning contract (e.g.: 1.00)
) 
RETURNS VOID AS $$
DECLARE
    total_group_winnings DECIMAL(18, 8);
    platform_fee_percent DECIMAL(5, 4) := 0.02; -- Example: 2% platform fee
    net_winnings DECIMAL(18, 8);
    member_record RECORD;
BEGIN
    -- 1. Calculate the group's gross winnings based on filled orders
    ...
    FOR member_record IN 
        ...
        WHERE group_id = target_group_id
    LOOP
        -- Credit the user's wallet ('users' table)
        UPDATE users 
    ...
    -- 4. Update final status of the group and orders
...
When should you call this function?

    Formation Failure: If the market starts in 5 minutes and the group has only reached 40% of target_liquidity. Your backend (Node.js/Python) should trigger a cron job that calls this function.

    Market Cancelled: If the market event was cancelled (e.g.: football game postponed or political event cancelled), you call this function for all active groups in that market_id.

    Execution Error: If the group order was not filled (e.g.: lack of liquidity in the global order book), the refund ensures that the money does not get stuck in limbo.

Postgres Security Considerations

    Referential Integrity: The group_members table has ON DELETE CASCADE. However, since we are dealing with money, never delete records. Always use the cancelled status.

    Transaction Control: Both Procedures (resolve and cancel) should be executed within a TRY/CATCH block in your backend to ensure that, if an UPDATE fails for any reason (e.g.: database down in the middle of the loop), the entire operation is rolled back.
...
DECLARE
    member_record RECORD;
BEGIN
    -- 1. Check if the group is already resolved or cancelled to avoid double refund
    ...
    -- 2. Loop to refund exactly the amount each member contributed
    -- Note that here there is no profit calculation, only the return of the principal capital
    FOR member_record IN 
        SELECT user_id, contribution_amount 
        FROM group_members 
        WHERE group_id = target_group_id
    LOOP
        -- Return the balance to the user's main wallet
        UPDATE users 
    ...
    -- 3. Update the group status to 'cancelled'
    UPDATE groups 
    SET status = 'cancelled', 
        description = description || ' | Cancellation Reason: ' || cancellation_reason
    WHERE id = target_group_id;

    -- 4. Cancel any pending orders linked to the group that were not filled
    UPDATE group_orders 
    SET status = 'cancelled' 
