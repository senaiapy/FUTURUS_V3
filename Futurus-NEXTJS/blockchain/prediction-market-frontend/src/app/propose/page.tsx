...
import {
  ...
  infoAlert,
  warningAlert,
} from "@/components/elements/ToastGroup";
...
export default function Propose() {
  ...
  const updateTokenTicker = async (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>,
  ) => {
    setTokenSearch(e.target.value);
    setNeededDataError(false);
    if (
      marketField[marketFieldIndex].content[marketFieldContentIndex]
        .api_name === "Coingecho"
    ) {
      ...
      if (isPubkey) {
        ...
        const result = await axios.get(
          marketField[marketFieldIndex].content[
            marketFieldContentIndex
          ].api_link(e.target.value),
        );
  ...
  const onSubmit = async () => {
    try {
      ...
      if (marketField[marketFieldIndex].name === "Sports Prediction Market") {
        ...
        const elem_val = document.getElementById(
          element.name,
        ) as HTMLInputElement;
        if (elem_val.value === "") {
          setNeededDataError(true);
      ...
      const task =
        market_detail.task(dexIndex, data.range) !== "null"
          ? market_detail.task(dexIndex, data.range)
          : findJsonPathsForKey(
              JSON.stringify(response.data),
              data.range ? "market_cap" : "usd",
            )[0];
      ...
      if (marketField[marketFieldIndex].name === "Sports Prediction Market") {
        data.question = `Will ${selectedTeam} ${selectedStatType
          .split("_")
          .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
          .join(" ")} reach ${data.value} by ${data.date}?`;
      } else {
        data.question = data.range
          ? `Will ${elipsKey(data.feedName)} reach a market cap of $ ${data.value} by ${data.date}?`
          : `Will ${elipsKey(data.feedName)} reach a per token price of $ ${data.value} by ${data.date}?`;
      }

      const res = await axios.post(`${url}api/market/create`, {
        data,
        isChecked,
      });
      ...
      const feed_result = await customizeFeed({
        url: data.dataLink,
        task,
        name: data.feedName,
        cluster,
      ...
      const update_res = await axios.post(`${url}api/market/add`, {
        data: { ...create_result, id: market_id },
      });
    ...
    <div className="px-[50px] flex-col 2xl:flex-row self-stretch inline-flex justify-start items-start gap-[50px] overflow-auto relative">
      <div className="flex-1 p-8 bg-[#1e1e1e] rounded-2xl outline-1 outline-offset-[-1px] outline-[#313131] inline-flex flex-col justify-center items-center gap-8 relative">
        {active ? (
          ...
          <div className="absolute flex justify-center items-center w-full h-full bg-[#1e1e1e]/50 backdrop-blur-sm z-20 rounded-2xl">
            <ClipLoader
              color="#ffffff"
              size={300}
        ...
        <div className="self-stretch flex flex-col gap-8">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col gap-4 lg:w-1/4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div
                  className={`text-red text-sm ${error.imageUrl !== "" ? "" : "invisible"}`}
                >
                  *Invalid Image
                </div>
              </div>
            ...
            <div className="flex flex-col gap-8 lg:w-3/4">
              ...
              <div className="flex flex-col gap-4 relative">
                ...
                {marketFieldOpen && (
                  <div
                    id="market_catagory"
                    className="w-full bg-[#111111] rounded-2xl outline-1 outline-offset-[-1px] absolute left-0 bottom-[-100px] z-10 outline-[#313131]"
                  >
              ...
              "Sports Prediction Market" ? (
                ...
                <div className="flex flex-col gap-4 relative">
                  ...
                  <button
                    className="w-full text-[#838587] px-4 py-3.5 text-lg font-medium bg-[#111111] rounded-2xl outline-1 outline-offset-[-1px] outline-[#313131] flex justify-between items-center hover:bg-[#1a1a1a] transition-colors"
                    onClick={() =>
                      setMarketFieldContentOpen(!marketFieldContentOpen)
                    }
                  >
                    {
                      marketField[marketFieldIndex].content[
                  ...
                  {marketFieldContentOpen && (
                    <div className="w-full bg-[#111111] rounded-2xl outline-1 outline-offset-[-1px] absolute left-0 bottom-[-100px] z-10 outline-[#313131]">
                      {marketField[marketFieldIndex].content.map(
                        (field, index) => (
                          <div
                            key={index}
                            className="px-4 py-3 hover:bg-[#1a1a1a] cursor-pointer transition-colors text-white"
          ...
          <div className="flex flex-col gap-8">
            <div className="text-white text-xl font-medium">
              Step 4: Build Your Prediction Question
            </div>
              ...
              >
                ...
                "Sports Prediction Market"
                  ...
                  : `Will ${data.feedName ? `${elipsKey(data.feedName)}` : "___"} ${
                      data.range === 0
                        ? "reach a per token price of $"
                        : data.range === 1
                          ? "reach a market cap of $"
                          : "___"
                    } ${
              ...
              selectedSport && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col gap-6">
                    {/* League Selection */}
                    <div className="relative">
                      <div className="text-[#838587] text-lg mb-2">
                        Select League
                      </div>
                      <button
                        className="w-full text-[#838587] px-4 py-3.5 text-lg font-medium bg-[#111111] rounded-2xl outline-1 outline-offset-[-1px] outline-[#313131] flex justify-between items-center hover:bg-[#1a1a1a] transition-colors"
                    ...
                    <div className="relative">
                      ...
                      {teamOpen && selectedLeague && (
                        <div className="absolute z-10 w-full mt-2 bg-[#111111] rounded-2xl outline-1 outline-offset-[-1px] outline-[#313131] shadow-lg">
                          {sportsData[selectedSport].teams[selectedLeague].map(
                            (team) => (
                              <div
                                key={team}
                                className="px-4 py-3 hover:bg-[#1a1a1a] cursor-pointer transition-colors text-white"
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex gap-4">
                          ...
                          className={`flex-1 px-4 py-3.5 text-lg font-medium rounded-2xl outline-1 outline-offset-[-1px] outline-[#313131] transition-colors ${
                            selectedStatType === "points"
                              ...
                              : !selectedTeam
                                ...
                                : "bg-[#111111] text-[#838587] hover:bg-[#1a1a1a] cursor-pointer"
                          }`}
                          onClick={() =>
                            selectedTeam && setSelectedStatType("points")
                          }
                        ...
                        <button
                          ...
                          onClick={() =>
                            selectedTeam && setSelectedStatType("next_game")
                          }
                          disabled={!selectedTeam}
                        >
                          Next Game
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      />
                      <div
                        className={`text-red ${error.value !== "" ? "" : "invisible"}`}
                      >
            ...
            {marketField[marketFieldIndex].name !==
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col gap-6">
                  <div className="flex flex-col gap-2">
                    <div className="text-[#838587] text-lg font-semibold">
                      Token Ticker
                    </div>
                    <div className="relative">
                      <input
                      ...
                      {tokenDropdownOpen && (
                        <div className="absolute z-10 w-full mt-2 max-h-60 overflow-y-auto bg-[#181a1b] rounded-2xl border border-[#232a32] shadow-lg">
                            ...
                            .map((token) => (
                              <div
                                ...
                                onClick={() => {
                                  ...
                                  setData((prev) => ({
                                    ...prev,
                                    feedName: token.symbol.toUpperCase(),
                                  }));
                                }}
                              >
                          ...
                          {tokenList.filter(
                            ...
                            <div className="px-4 py-3 text-[#838587]">
                              No tokens found
                            </div>
                          )}
                        </div>
                    ...
                    <div
                      className={`text-red ${needDataError ? "" : "invisible"}`}
                    >
                      *Please enter a token ticker
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-6">
                  <div className="flex flex-row gap-4">
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                      ...
                      />
                      <div
                        className={`text-red ${error.date !== "" ? "" : "invisible"}`}
                      >
            ...
            <div className="flex flex-col gap-2 mt-6">
              ...
              />
              <div
                className={`text-red ${error.description !== "" ? "" : "invisible"}`}
              >
            ...
            <div className="flex flex-col gap-6 mt-4">
              ...
              <div
                className={`text-red ${error.checkbox !== "" ? "" : "invisible"}`}
              >
