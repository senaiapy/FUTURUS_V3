...
type FutCounterProps = {
  setAmount: React.Dispatch<React.SetStateAction<number>>;
  amount: number;
};

const FutCounter = ({ setAmount, amount }: FutCounterProps) => {
  const increaseFut = () => setAmount((prev: number) => prev + 1);
  const decreaseFut = () => {
    if (amount > 0) {
      setAmount((prev: number) => Math.max(prev - 1, 0));
    }
  };
  ...
  return (
    <div className="self-stretch p-2 bg-[#111111] rounded-xl shadow-[inset_0px_2px_0px_0px_rgba(0,0,0,0.20)] outline-1 outline-offset-[-1px] outline-[#313131] inline-flex justify-start items-center gap-3">
      {/* Decrease Button */}
      <div
        onClick={amount > 0 ? decreaseFut : undefined}
      ...
      <div className="flex-1 rounded-lg flex justify-center items-center gap-2">
        <input
          type="number"
          className="justify-start text-[#838587] text-xl text-center font-medium font-satoshi leading-tight outline-none"
          value={amount}
