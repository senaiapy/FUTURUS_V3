...
###############################################################################
# FUTURUS - COMPLETE SERVER UPLOAD & DEPLOYMENT SCRIPT
#
# Interactive script for building, packaging, uploading, and deploying
# the Futurus stack to a remote server.
#
# Features:
...
# - Compress images and volumes for transfer
...
if ask_yes_no "Do you want to create a database backup before deployment?"; then
    ...
    if docker ps --filter "name=futurus-db" --filter "status=running" -q 2>/dev/null | grep -q .; then
        ...
        # Try PostgreSQL
        elif docker exec futurus-db pg_dump -U "${DB_USERNAME:-galo}" -d "${DB_DATABASE:-futurus_fullstack}" > "$BACKUP_DIR/$BACKUP_FILE" 2>/dev/null; then
            gzip "$BACKUP_DIR/$BACKUP_FILE"
            print_success "Backup created at $BACKUP_DIR/${BACKUP_FILE}.gz"
        else
...
read -p "Port for database [3306]: " DB_PORT
...
if ask_yes_no "Do you want to erase all old Futurus Docker images locally?"; then
    ...
    docker rmi futurus-laravel-app:latest 2>/dev/null || true
...
if ask_yes_no "Do you want to build and compile the Docker images?" "y"; then
    ...
    if docker buildx build --platform "$PLATFORM" --provenance=false -t futurus-laravel-app:latest -f Dockerfile.prod --load . 2>&1 | while read line; do
        echo "  $line"
    done; then
    ...
    BUILT_ARCH=$(docker image inspect futurus-laravel-app:latest --format '{{.Architecture}}' 2>/dev/null || echo "unknown")
...
if ask_yes_no "Do you want to export and compress Docker images for transfer?" "y"; then
    ...
    docker save futurus-laravel-app:latest | gzip > "$EXPORT_DIR/images/futurus-app.tar.gz"
    ...
    docker pull --platform linux/amd64 mariadb:10.6 2>/dev/null || true
    ...
    if ask_yes_no "Do you want to also export database volumes (for data migration)?"; then
        print_step "Exporting database volume..."
        VOLUME_NAME=$(docker volume ls -q | grep -E "futurus.*db|futurus.*data" | head -1)
        if [ -n "$VOLUME_NAME" ]; then
            docker run --rm -v "$VOLUME_NAME":/data -v "$EXPORT_DIR/volumes":/backup alpine tar czf /backup/db-volume.tar.gz -C /data .
            print_success "Volume exported: $(du -h "$EXPORT_DIR/volumes/db-volume.tar.gz" | cut -f1)"
        else
            print_warning "No database volume found to export."
        fi
    fi
...
#-----------------------------------------------
print_header "7. RESTORE DATABASE BACKUP"
...
services:
  app:
    image: futurus-laravel-app:latest
    container_name: futurus-app-prod
    restart: always
    ports:
      - "${APP_PORT}:80"
    environment:
...
if ask_yes_no "Ready to upload files to $SERVER_IP. Continue?" "y"; then
    ...
    if mkdir -p ${SERVER_PATH}/images ${SERVER_PATH}/SQL ${SERVER_PATH}/backups ${SERVER_PATH}/volumes 2>/dev/null; then
        echo 'Directories created successfully'
    else
        # Try with sudo
        echo 'Permission denied, trying with sudo...'
        sudo mkdir -p ${SERVER_PATH}/images ${SERVER_PATH}/SQL ${SERVER_PATH}/backups ${SERVER_PATH}/volumes
    ...
    OLD_IMAGES=\$(sudo docker images -q futurus-laravel-app 2>/dev/null || true)
    ...
    # Upload volumes if exported
    if [ -d "$EXPORT_DIR/volumes" ] && [ "$(ls -A "$EXPORT_DIR/volumes" 2>/dev/null)" ]; then
        print_step "Uploading volume data..."
        run_scp "$EXPORT_DIR/volumes/" "${SERVER_PATH}/"
    fi
...
# Remove existing images before loading new ones
echo 'Removing any existing images...'
sudo docker rmi -f futurus-laravel-app:latest 2>/dev/null || true
...
if sudo docker image inspect futurus-laravel-app:latest &>/dev/null; then
    echo 'App image loaded successfully!'
    IMG_ARCH=\$(sudo docker image inspect futurus-laravel-app:latest --format '{{.Architecture}}' 2>/dev/null || echo 'unknown')
...
sudo docker images | grep -E 'futurus|mariadb'
"
...
sudo docker exec futurus-app-prod sed -i \"s/^APP_ENV=.*/APP_ENV=production/\" /var/www/html/core/.env
sudo docker exec futurus-app-prod sed -i \"s/^APP_DEBUG=.*/APP_DEBUG=false/\" /var/www/html/core/.env
sudo docker exec futurus-app-prod sed -i \"s|^APP_URL=.*|APP_URL=${FINAL_APP_URL}|\" /var/www/html/core/.env

# Clear caches
echo 'Clearing caches...'
sudo docker exec futurus-app-prod rm -f /var/www/html/core/bootstrap/cache/*.php 2>/dev/null || true
sudo docker exec futurus-app-prod php /var/www/html/core/artisan key:generate --force 2>/dev/null || true
sudo docker exec futurus-app-prod php /var/www/html/core/artisan config:clear 2>/dev/null || true
sudo docker exec futurus-app-prod php /var/www/html/core/artisan cache:clear 2>/dev/null || true
sudo docker exec futurus-app-prod php /var/www/html/core/artisan view:clear 2>/dev/null || true

# Set permissions
echo 'Setting permissions...'
sudo docker exec futurus-app-prod chown -R www-data:www-data /var/www/html/core/storage 2>/dev/null || true
sudo docker exec futurus-app-prod chmod -R 775 /var/www/html/core/storage 2>/dev/null || true
