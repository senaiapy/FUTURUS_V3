...
import { Card, Badge } from "@/components/ui/PremiumUI";
...
interface WithdrawalFilterPageProps {
  title: string;
  subtitle: string;
  filter: string;
  emptyMessage?: string;
...
export default function WithdrawalFilterPage({
  title,
  subtitle,
  filter,
  emptyMessage = "Nenhuma solicitação encontrada",
}: WithdrawalFilterPageProps) {
    ...
    <div className="space-y-10 pb-12">
      ...
      <div className="flex flex-col lg:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest bg-white/3 px-6 py-3 rounded-2xl border border-white/5">
            {loading ? "..." : filteredWithdrawals.length} retiradas
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          {
            label: "Total",
            value: `R$${withdrawals.reduce((acc, w) => acc + Number(w.amount || 0), 0).toFixed(2)}`,
            icon: Wallet,
      ...
      {/* Search */}
      <div className="bg-[#141726]/60 border border-white/5 p-3 rounded-[32px] flex items-center gap-4 shadow-2xl backdrop-blur-md">
