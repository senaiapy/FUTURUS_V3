...
const getLatestRelease = async () => {
  try {
    const repoData = await fetch(
      'https://api.github.com/repos/pyfoundation/react-native-template-pyfoundation/releases/latest'
    );
...
const cloneLastTemplateRelease = async (projectName) => {
  ...
  consola.info(`Using Pyfoundation starter ${latest_release}`);

  // create a new project based on pyfoundation template
  const cloneStarter = `git clone -b ${latest_release} --depth=1   https://github.com/pyfoundation/react-native-template-pyfoundation.git ${projectName}`;
