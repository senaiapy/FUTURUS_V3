...
function getImage($image, $size = null, $avatar = false, $option = false) {
    $clean = '';
    ...
    if ($option) {
        return asset('assets/images/option.png');
    }
...
function getRealIP() {
    // SECURITY FIX: Prioritize trusted headers only
    // Order matters: most trusted first (Cloudflare > nginx X-Real-IP > REMOTE_ADDR)
    // Other headers (HTTP_X_FORWARDED_FOR, HTTP_CLIENT_IP) are easily spoofable

    $ip = $_SERVER["REMOTE_ADDR"] ?? '127.0.0.1';

    // Cloudflare - most trusted if you use Cloudflare
    if (!empty($_SERVER['HTTP_CF_CONNECTING_IP']) &&
        filter_var($_SERVER['HTTP_CF_CONNECTING_IP'], FILTER_VALIDATE_IP)) {
        $ip = $_SERVER['HTTP_CF_CONNECTING_IP'];
    }
    // Nginx reverse proxy with X-Real-IP configured
    elseif (!empty($_SERVER['HTTP_X_REAL_IP']) &&
        filter_var($_SERVER['HTTP_X_REAL_IP'], FILTER_VALIDATE_IP)) {
        $ip = $_SERVER['HTTP_X_REAL_IP'];
    }
    // Note: HTTP_X_FORWARDED_FOR, HTTP_CLIENT_IP, HTTP_FORWARDED are NOT trusted
    // as they can be easily spoofed by clients. Only use if behind trusted proxy.
