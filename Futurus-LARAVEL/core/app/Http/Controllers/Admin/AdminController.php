...
class AdminController extends Controller {
    ...
    public function dashboard() {
        ...
        $purchase['today']      = Purchase::whereDate('created_at', $today)->sum('amount');
        $purchase['this_week']  = Purchase::whereBetween('created_at', [$startWeek, $endWeek])->sum('amount');
        $purchase['this_month'] = Purchase::whereBetween('created_at', [$startMonth, $endMonth])->sum('amount');
        $purchase['all_time']   = Purchase::sum('amount');

        $market['total_market']    = Market::active()->count();
        $market['live_market']     = Market::running()->count();
        $market['resolved_market'] = Market::where('status', Status::MARKET_SETTLED)->count();
        $market['upcoming_market'] = Market::where('start_date', '>', now())->count();

        $profit['all_profit'] = Market::where('status', Status::MARKET_SETTLED)
            ->sum('profit_loss');

        $profit['today_profit'] = Market::where('status', Status::MARKET_SETTLED)
            ->whereDate('resolved_at', $today)
            ->sum('profit_loss');

        $profit['this_week_profit'] = Market::where('status', Status::MARKET_SETTLED)
            ->whereBetween('resolved_at', [$startWeek, $endWeek])
            ->sum('profit_loss');

        $profit['this_month_profit'] = Market::where('status', Status::MARKET_SETTLED)
            ->whereBetween('resolved_at', [$startMonth, $endMonth])
            ->sum('profit_loss');

        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart', 'deposit', 'withdrawals', 'market', 'purchase', 'profit'));
    }

    public function profitPurchase() {
        $labels    = [];
        $purchases = [];
        $profits   = [];
        ...
        for ($i = 29; $i >= 0; $i--) {
            ...
            $purchases[] = Purchase::whereDate('created_at', $date)->sum('amount');
            $profits[]   = Market::where('status', Status::MARKET_SETTLED)
    ...
    public function marketStatus() {
        $data = [
            'Live'      => Market::running()->count(),
            'Resolved'  => Market::declared()->count(),
            'Cancelled' => Market::cancelled()->count(),
            'Pending'   => Market::closed()->count(),
        ];
    ...
    public function activeUsers() {
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $series[] = UserLogin::whereDate('created_at', $date)
                ->distinct('user_id')
                ->count('user_id');
        }
    ...
    public function show2faForm()
    {
        ...
        return view('admin.twofactor', compact('pageTitle', 'secret', 'qrCodeUrl', 'admin'));
    }
    ...
    public function disable2fa(Request $request)
    {
        ...
        if ($response) {
            ...
            $admin->ts = 0;
            $admin->save();
            $notify[] = ['success', 'Two factor authentication disabled successfully'];
            return back()->withNotify($notify);
        } else {
            $notify[] = ['error', 'Wrong verification code'];
            return back()->withNotify($notify);
    ...
    public function verify2faView()
    {
        ...
        return view('admin.auth.2fa', compact('pageTitle'));
    }

    public function verify2faSubmit(Request $request)
    {
        $request->validate([
            'code' => 'required',
        ]);
