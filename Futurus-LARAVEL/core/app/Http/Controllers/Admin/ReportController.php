...
class ReportController extends Controller {
    ...
    public function purchaseHistory($userId = null) {
        ...
        $purchases = Purchase::with('market', 'option', 'user')->latest()->searchable(['trx', 'market:question', 'option:question', 'user:username'])->dateFilter();

        if ($userId) {
            $purchases = $purchases->where('user_id', $userId);
        }

        $purchases = $purchases->paginate(getPaginate());

        return view('admin.reports.purchase', compact('pageTitle', 'purchases'));
    }
