...
trait SupportTicketManager {
    ...
    public function storeSupportTicket(Request $request) {
        ...
        // SECURITY FIX: Use cryptographically secure random string instead of predictable rand()
        $ticket->ticket     = strtoupper(Str::random(12));
                    ...
                    foreach ($this->files as $file) {
                        if ($file && $file->isValid()) {
                            ...
                            if (!isset($allowedMimes[$ext]) || !in_array($mimeType, $allowedMimes[$ext])) {
                                return $fail("File type mismatch detected. Please upload a valid file.");
                            }

                            // Check file size (max 5MB per file)
                            if ($file->getSize() > 5 * 1024 * 1024) {
                                return $fail("Each file must be less than 5MB");
                            }
                        }
                    }
    ...
    public function ticketDownload($attachmentId) {
        ...
        // SECURITY FIX: Verify user owns the ticket before allowing download
        $user = $this->user;
        $column = $this->column;
        $ticket = $attachment->supportMessage->ticket ?? null;
        if ($ticket && $this->userType != 'admin') {
            if (!$user || $ticket->$column != $user->id) {
                if ($this->apiRequest) {
