...
trait UserNotify {
    public static function notifyToUser() {
        return [
            'allUsers'               => 'All Users',
            'userWithPurchases'      => 'Users With Purchases',
            'userWithoutPurchases'   => 'Users Without Purchases',
            'winningUsers'           => 'Users With Winning Purchases',
            'losingUsers'            => 'Users With Losing Purchases',
            'topProfitableUsers'     => 'Top Profitable Users',
    ...
    public function scopeUserWithPurchases($query) {
        return $query->whereHas('purchases');
    }

    public function scopeUserWithoutPurchases($query) {
        return $query->whereDoesntHave('purchases');
    }

    public function scopeWinningUsers($query) {
        return $query->whereHas('purchases', function ($q) {
            $q->where('status', Status::MARKET_SETTLED)
                ->where('win', Status::YES);
        });
    }

    public function scopeLosingUsers($query) {
        return $query->whereHas('purchases', function ($q) {
            $q->where('status', Status::MARKET_SETTLED)
                ->where('win', Status::NO);
        });
    ...
    public function scopeTopProfitableUsers($query) {
        return $query->whereHas('purchases', function ($q) {
            $q->where('status', Status::MARKET_SETTLED);
        })
