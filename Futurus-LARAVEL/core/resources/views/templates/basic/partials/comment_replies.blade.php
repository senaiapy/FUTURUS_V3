        ...
        <div class="comment-item__thumb">
            @if ($reply?->user?->image)
                <img class="fit-image"
                     ...
                     alt="comment">
            @else
                <x-user-initials :user="$reply?->user" />
            @endif
        </div>
        <div class="comment-item__content">
            <div class="comment-item__top">
                <p class="comment-item__name">{{ $reply?->user?->fullname }}</p>
                <span>•</span>
                <span class="comment-item__date">{{ diffForHumans($reply->created_at) }}</span>
            </div>
            <p class="comment-item__text">{{ $reply->comment }}</p>
            <div class="flex-align gap-3">
                <button class="comment-item__like like-btn {{ $reply->authUserLike ? 'liked' : '' }}"
                ...
                <button class="comment-item__reply reply-btn" data-comment-id="{{ $reply->id }}">
                    @lang('Reply')
                </button>
            </div>
            <div class="comment-form-wrapper">
                <form action="#" class="comment-form reply-form" data-parent-id="{{ $reply->parent_id }}">
                    <textarea class="comment-form__input" placeholder="Write a reply..."></textarea>
        ...
        @if ($reply?->user?->id != auth()->id())
            <div class="dropdown action-dropdown">
                ...
                <ul class="dropdown-menu overflow-hidden p-0">
                    <li>
                        <a class="dropdown-item report-btn" href="#" data-comment-id="{{ $reply->id }}">
