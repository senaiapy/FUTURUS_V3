            ...
            <div class="card custom--card">
                <div class="card-body">
                    ...
                    <form action="{{route('user.withdraw.submit')}}" method="post" enctype="multipart/form-data">
                        ...
                        @if($withdraw->withdraw_information && is_array($withdraw->withdraw_information))
                            <div class="alert alert-info mb-3">
                                <h6 class="mb-2">@lang('PIX Information')</h6>
                                @foreach($withdraw->withdraw_information as $info)
                                    @php
                                        $infoData = is_object($info) ? $info : (object)$info;
                                    @endphp
                                    <p class="mb-1"><strong>{{ $infoData->name }}:</strong> {{ $infoData->value }}</p>
                                @endforeach
                            </div>
                        @endif
