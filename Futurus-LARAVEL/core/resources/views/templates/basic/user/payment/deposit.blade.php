                                    ...
                                    @foreach ($gatewayCurrency as $data)
                                        <label for="{{ titleToKey($data->name) }}" class="payment-item @if ($loop->index > 4) d-none @endif gateway-option">
                                            ...
                                            <div class="payment-item__thumb">
                                                @php
                                                    $image = getImage(getFilePath('gateway') . '/' . $data->method->image);
    ...
    <script>
        ...
        (function($) {
            ...
            function gatewayChange() {
                ...
                // Show CPF field for Asaas PIX and Asaas Card
                if (gateway.gateway_alias == 'Asaas' || gateway.gateway_alias == 'AsaasCard' || gateway.method_code == 127 || gateway.method_code == 128) {
                    $('.cpf-cnpj-field').removeClass('d-none');
                    $('.cpf_cnpj').attr('required', true);
                } else {
                    $('.cpf-cnpj-field').addClass('d-none');
