...
@section('panel')
    ...
    <div class="row gy-4 mt-2">
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2"
                      link="{{ route('admin.market.declared') }}?date={{ now()->format('F+d,%Y') }}+-+{{ now()->format('F+d,%Y') }}"
                      title="Today's Profit/Loss" icon="las la-calendar-day" value="{{ showAmount($profit['today_profit']) }}"
                      color="green" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2"
                      link="{{ route('admin.market.declared') }}?date={{ now()->startOfWeek()->format('F+d,%Y') }}+-+{{ now()->endOfWeek()->format('F+d,%Y') }}"
                      title="This Week's Profit/Loss" icon="las la-calendar-week"
                      value="{{ showAmount($profit['this_week_profit']) }}" color="info" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2"
                      link="{{ route('admin.market.declared') }}?date={{ now()->startOfMonth()->format('F+d,%Y') }}+-+{{ now()->endOfMonth()->format('F+d,%Y') }}"
                      title="This Month's Profit/Loss" icon="las la-calendar-alt"
                      value="{{ showAmount($profit['this_month_profit']) }}" color="purple" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2" link="{{ route('admin.market.declared') }}" title="All Time Profit/Loss"
                      icon="las la-chart-line" value="{{ showAmount($profit['all_profit']) }}" color="blue" />
        </div>
    </div>
    ...
    <div class="row gy-4 mt-2">
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2"
                      link="{{ route('admin.report.purchase.history') }}?search=&date={{ now()->format('F+d,%Y') }}+-+{{ now()->format('F+d,%Y') }}"
                      title="Today's Purchases" icon="las la-shopping-bag" value="{{ showAmount($purchase['today']) }}"
                      color="success" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2"
                      link="{{ route('admin.report.purchase.history') }}?search=&date={{ now()->startOfWeek()->format('F+d,%Y') }}+-+{{ now()->endOfWeek()->format('F+d,%Y') }}"
                      title="This Week's Purchases" icon="las la-shopping-basket"
                      value="{{ showAmount($purchase['this_week']) }}" color="warning" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2"
                      link="{{ route('admin.report.purchase.history') }}?search=&date={{ now()->startOfMonth()->format('F+d,%Y') }}+-+{{ now()->endOfMonth()->format('F+d,%Y') }}"
                      title="This Month's Purchases" icon="las la-clipboard-list"
                      value="{{ showAmount($purchase['this_month']) }}" color="info" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2" link="{{ route('admin.report.purchase.history') }}" title="All Time Purchases"
                      icon="las la-warehouse" value="{{ showAmount($purchase['all_time']) }}" color="primary" />
        </div>
    </div>

    <div class="row gy-4 mt-2">
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2" link="{{ route('admin.market.index') }}" icon="las la-chart-line"
                      title="Total Markets" value="{{ $market['total_market'] }}" color="primary" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2" link="{{ route('admin.market.live') }}" icon="las la-broadcast-tower"
                      title="Live Markets" value="{{ $market['live_market'] }}" color="success" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2" link="{{ route('admin.market.declared') }}" icon="las la-check-circle"
                      title="Resolved Markets" value="{{ $market['resolved_market'] }}" color="info" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="2" link="{{ route('admin.market.upcoming') }}" title="Upcoming Markets"
                      icon="las la-hourglass-start" value="{{ $market['upcoming_market'] }}" color="warning" type="2" />
        </div>
    </div>
    ...
    <div class="row mt-2 gy-4">
        <div class="col-xl-4 mb-30">
            <div class="card h-100">
                <div class="card-body">
                    <h5 class="card-title">@lang('Profit vs Purchases (Last 30 days)')</h5>
                    <div id="profitPurchaseChart"></div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xl-4 mb-30">
            <div class="card h-100">
                <div class="card-body">
                    <h5 class="card-title">@lang('Market Status Breakdown')</h5>
        ...
        <div class="col-xl-4 mb-30">
            <div class="card h-100">
                <div class="card-body">
                    <h5 class="card-title">@lang('Active Users (Last 7 days)')</h5>
...
@push('breadcrumb-plugins')
    <button type="button" class="btn btn-outline--primary btn-sm" data-bs-toggle="modal" data-bs-target="#cronModal">
...
@push('script')
    <script>
        ...
        $.get(`{{ route('admin.chart.profit.purchase') }}`, function(data) {
            lineChart(
                document.querySelector("#profitPurchaseChart"),
                data.data,
                data.labels
            );
        });

        $.get(`{{ route('admin.chart.market.status') }}`, function(data) {
            piChart(
                document.querySelector("#marketStatusChart"),
                data.labels,
                data.series
            );
        });

        $.get(`{{ route('admin.chart.active.users') }}`, function(data) {
            barChart(
                document.querySelector("#activeUsersChart"),
                "Users",
                [{
                    name: "Active Users",
                    data: data.series
